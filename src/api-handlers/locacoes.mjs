...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (finalId) {
        ...
        if (locacaoError) {
          ...
          return response.status(500).json({ success: false, error: "Erro ao buscar dados da locação." });
        }
    ...
    if (method === 'POST') {
      ...
      // Inserir registro financeiro para o seguro, se houver valor
      if (newLocacao && newLocacao.valor_seguro > 0) {
        const { error: seguroMovError } = await supabase.from('movimentacoes_financeiras').insert({
          tipo: 'entrada',
          categoria: 'seguro',
          descricao: `Recebimento de Seguro - Locação #${newLocacao.id}`,
          valor: newLocacao.valor_seguro,
          data_movimentacao: newLocacao.data_locacao,
          locacao_id: newLocacao.id,
          cliente_id: newLocacao.cliente_id,
        });
        ...
        if (seguroMovError) {
          console.error('ALERTA: A locação foi criada, mas falhou ao inserir o registro financeiro do seguro:', seguroMovError);
          // Não retorna erro para o cliente, mas loga o problema para manutenção
        }
      }
